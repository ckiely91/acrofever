...
Meteor.startup(function() {
  ...
  _.each(DefaultLobbies, lobby => {
    ...
    if (!insertedLobby.currentGame) {
      ...
      const game = Games.findOne(insertedLobby.currentGame, {
        fields: { active: true }
      });
      ...
      if (active) {
        ...
        LobbyManager.addSystemMessage(
          insertedLobby._id,
          "Sorry, the current game was cancelled because of a server restart.",
          "warning",
          "Please rejoin the lobby to start a new game."
        );
...
Meteor.onConnection(connection => {
  if (
    ...
    BannedIPs.findOne({ ip: connection.clientAddress })
  ) {
