...
Accounts.onCreateUser(function(options, user) {
  ...
  if (userDetails.email && !Meteor.settings.development) {
    HTTP.call(
      "POST",
      "https://" +
        Meteor.settings.mailchimp.dataCenter +
        ".api.mailchimp.com/3.0/lists/" +
        Meteor.settings.mailchimp.listId +
        "/members/",
      {
        data: {
        ...
        auth:
          Meteor.settings.mailchimp.username +
          ":" +
          Meteor.settings.mailchimp.apiKey
      },
  ...
  if (Meteor.settings.blacklistedEmailDomains && userDetails.email) {
    ...
    if (
      match &&
      match[1] &&
      Meteor.settings.blacklistedEmailDomains.indexOf(match[1]) > -1
    ) {
