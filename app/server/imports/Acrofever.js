    ...
    if (winners.length > 1) {
        ...
        if (newWinners.length === 1) {
            ...
            _.each(newWinners, id => {
                let totalTimes = 0;
                const sumTime = game.rounds.reduce((val, round) => {
                    if (round.players[id] && round.players[id].submission) {
                        totalTimes++;
                        return val + round.players[id].submission.timeLeft;
                    } else {
                        return val;
                    }
                }, 0);

                const avgTime = sumTime / totalTimes;

                if (avgTime <= lowestTime) {
                    newNewWinners.push(id);
                    lowestTime = avgTime;
                } else {
                    const thisPlayerIndex = newNewWinners.indexOf(id);
                    if (thisPlayerIndex > -1) {
                        newNewWinners.splice(thisPlayerIndex, 1);
                    }
                }
            });
