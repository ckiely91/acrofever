...
import { displayName, isUserBanned } from "../../imports/helpers";
...
Meteor.methods({
  joinOrLeaveOfficialLobby(lobbyId, join) {
    ...
    if (typeof join !== "undefined") check(join, Boolean);

    if (!this.userId)
      throw new Meteor.Error("403", "You must be logged in to do that");

    let lobby = Lobbies.findOne(lobbyId);
    if (!lobby || !lobby.official) throw new Meteor.Error("No valid lobby");

    let username;
    if (join) {
      // Check if banned
      if (isUserBanned(this.userId))
      ...
      LobbyManager.addSystemMessage(
        null,
        username + " joined the lobby " + lobby.displayName
      );
      ...
      if (
        !game.active &&
        !lobby.newGameStarting &&
        lobby.players.length >= Meteor.settings[game.type].minimumPlayers
      ) {
      ...
      LobbyManager.addSystemMessage(
        null,
        username + " left the lobby " + lobby.displayName
      );
  ...
  registerForReminder(eventId, deregister) {
    ...
    if (typeof deregister !== "undefined") check(deregister, Boolean);
    ...
    if (!event) throw new Meteor.Error("no-event", "That event does not exist");
    ...
    if (
      ...
      throw new Meteor.Error(
        "no-email-address",
        "You must have an email address set to do that"
      );
    ...
    switch (statType) {
      case "gamesPlayed":
        ...
        _.each(games, game => {
          ...
          if (stats[day]) {
            stats[day].played++;
            if (game.gameWinner === userId) stats[day].won++;
          } else {
            ...
            if (keys.length > 0) {
              ...
              else stats[day].won = prevStats.won;
            } else {
              stats[day].played = 1;
              if (game.gameWinner === userId) stats[day].won = 1;
              else stats[day].won = 0;
            }
          }
        });
      ...
      case "averageScoreAndRating":
        ...
        var games = Games.find(selector, {
          fields: { created: true, scores: true }
        });
      ...
      case "ranking":
        ...
        let i = 0;
        for (; i < total; i++) {
          if (users[i]._id === userId) return { total, rank: i + 1 };
        }
  ...
  inviteToPlay(userId, lobbyId) {
    ...
    if (!lobby) throw new Meteor.Error("lobby-not-found", "Lobby not found");
    ...
    if (user.invitesSent && user.invitesSent[this.userId]) {
      ...
      if (moment(user.invitesSent[this.userId]).isAfter(thresh)) {
        throw new Meteor.Error(
          "too-many-invites",
          "You've already sent an invite to this user."
        );
      }
    }
  ...
  changeUsername(username) {
    ...
    if (
      Meteor.users
        .find({ _id: { $ne: this.userId }, username: username })
        .count() > 0
    )
  ...
  removeFriend(userId) {
    ...
    const thisUser = Meteor.users.findOne(this.userId, {
      fields: { "profile.friends": true }
    });

    if (
      thisUser.profile &&
      thisUser.profile.friends &&
      thisUser.profile.friends.indexOf(userId) === -1
    )
